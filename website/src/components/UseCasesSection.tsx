    ...
    <section className="flex flex-col items-center gap-16 w-full px-20 py-[100px] bg-[var(--bg-light)]">
      ...
      <div className="grid grid-cols-2 gap-6 w-full">
        {useCases.map((uc) => (
          <div
            key={uc.title}
            className="flex flex-col gap-5 p-8 rounded-2xl bg-white border border-[var(--border-light)]"
          >
            <div
              className={`flex items-center justify-center w-12 h-12 rounded-xl ${uc.iconBg}`}
            >
              {uc.icon}
            </div>
            <div className="w-full h-[140px] rounded-[10px] overflow-hidden">
              <img
                src={uc.img}
                alt={uc.title}
