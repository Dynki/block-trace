...
export default function VisionSection() {
  return (
    <section className="flex flex-col items-center gap-8 md:gap-12 w-full px-5 sm:px-8 lg:px-20 py-16 md:py-[100px] bg-[var(--bg-light)]">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 w-full">
        {visionCards.map((card) => (
          <div
            key={card.num}
            className="flex flex-col gap-3 p-6 md:p-8 rounded-xl"
          >
            <span className="font-display text-3xl md:text-4xl font-extrabold text-[#E4E4E7]">
              {card.num}
            </span>
            <span className="font-display text-base md:text-lg font-bold text-[var(--text-dark)]">
              {card.title}
            </span>
