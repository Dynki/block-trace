...
export default function EnterprisePlatformSection() {
  return (
    <section className="flex items-center gap-20 w-full px-20 py-[100px] bg-[var(--bg-dark)]">
      {/* Left */}
      <div className="flex flex-col gap-8 flex-1">
        <span className="text-xs font-semibold text-[var(--accent-blue-light)] tracking-[2px]">
          ENTERPRISE PLATFORM
        </span>
        <h2 className="font-display text-[44px] font-extrabold text-[var(--text-white)] leading-[1.1] tracking-[-1.5px]">
          Infrastructure, Not Just Software
        </h2>
        <p className="text-[17px] text-[var(--text-white-secondary)] leading-[1.7]">
      ...
      <div className="flex flex-col gap-4 flex-1">
        {bizCards.map((card) => (
          <div
            key={card.title}
            className="flex items-center gap-4 p-6 rounded-xl bg-[var(--bg-dark-elevated)] border border-[var(--border-dark)]"
          >
            <div className="flex items-center justify-center w-11 h-11 rounded-[10px] bg-[#1E3A5F] shrink-0">
              {card.icon}
            </div>
            <div className="flex flex-col gap-1 flex-1">
              ...
              <span className="text-[13px] text-[var(--text-white-tertiary)] leading-[1.5]">
                {card.desc}
              </span>
            </div>
          </div>
